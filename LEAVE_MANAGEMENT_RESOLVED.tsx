...
export default function LeaveManagement({ employee, onUpdate, preLoadedLeaves, leaveBalance: preLoadedBalance }) {
  ...
  // Calculate leave balance using business days (excluding weekends)
  ...
  // Calculate days using business days calculator
  ...
  const handleSubmit = async (e) => {
    ...
    if (leaveTypesWithBalance.includes(formData.leave_type) && daysRequested > leaveBalance) {
      setFormError(
        `Insufficient leave balance. Available: ${leaveBalance} days, Requested: ${daysRequested} days.`,
      );
      return;
    }
  ...
  // Get supervisor name with multiple fallbacks
  const getSupervisorName = () => {
    if (employee?.supervisor) {
      return `${employee.supervisor.firstName || employee.supervisor.first_name} ${
        employee.supervisor.lastName || employee.supervisor.last_name
      }`;
    }
    ...
    if (employee?.supervisorId && employees.length > 0) {
      ...
      if (supervisor)
        return `${supervisor.firstName || supervisor.first_name} ${supervisor.lastName || supervisor.last_name}`;
    }
    return 'N/A';
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-6 pt-4">
            <Alert variant="info" className="bg-orange-50 border-orange-200">
              ...
              <AlertDescription className="text-orange-800">
                Please fill out all relevant fields. While not mandatory, complete information helps expedite the
                approval process.
              </AlertDescription>
            </Alert>
